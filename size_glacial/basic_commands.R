...
table(data$AGE)
#basic stats
filename<-paste0(path,taxon,"_basic.csv")
...
write.table(TSfit4.wt,paste0(path,taxon,"TS.csv"),row.names = F)
#TS plots
source("read.paleoTS.R") #for some reason the function is not loading with the package; this is from github
par(mfrow=c(1,2),mar=c(2,3,3,1),oma=c(1,0,2,0)); for (i in 1:2) {
  plot.paleoTS(dataTS[[i]],main=tolower(names(dataTS)[i]),ylim=range(data[,i+1]))
  points(data[,1],data[,i+1])
}; mtext(paste0(taxon),outer=TRUE,cex=1.5)
#significance tests
correction<-0.05/length(levels(age.fac))
KWtest<-array(dim=c(2,4));rownames(KWtest)<-names(data[-1]);colnames(KWtest)<-c("K-W chi-squared","df","p-value",paste0("significant (p<",correction,")"))
for (i in 1:2) {
...
MWboot<-array(dim=c(length(levels(age.fac)),2));colnames(MWboot)<-names(data)[-1];rownames(MWboot)<-levels(age.fac)
for (j in 1:2){
...
MWresults<-cbind(MWx[,"LENGTH"],MWboot[,"LENGTH"],MWx[,"WIDTH"],MWboot[,"WIDTH"]);colnames(MWresults)<-c("Length.U","Length.p","Width.U","Width.p")
