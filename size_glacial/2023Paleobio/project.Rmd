...
```{r}
dimnamelist <- vector(mode = "list",length = length(meas))
names(dimnamelist) <- names(meas)
for (m in 1:length(dimnamelist)) {
  dimnamelist[[m]] <- vector(mode = "list", length = length(meas[[m]]))
  names(dimnamelist[[m]]) <- names(meas[[m]])
...
save(dimnamelist,meas,measpc,climateBins,halfBins,file = "objects.R")
```
...
TSboot5 <- list_flatten(list_flatten(TSboot5))
...
for (m in 1:length(TSboot5.pca)) {
  for (n in 1:length(TSboot5.pca[[m]])) {
    names(TSboot5.pca[[m]][[n]]) <- dimnamelist[[m]][[n]]
  }
}
...
for (m in 1:length(TSboot10.pca)) {
  for (n in 1:length(TSboot10.pca[[m]])) {
    names(TSboot10.pca[[m]][[n]]) <- dimnamelist[[m]][[n]]
  }
}
...
combineRepsTS(TSboot5[[i]]) -> test

for (i in 1:100) {
  TSbootmodels5[[1]][[1]][[1]][[1]]$compare$Akaike.wt
}
```
