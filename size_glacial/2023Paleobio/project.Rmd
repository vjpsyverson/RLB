...
```{r}
newbirds <- list(buteo_regalis_tmt = read.xlsx(here("newbirds/Buteo_regalis.xlsx"),sheet = "data"),
                 buteogallus_fragilis_tmt = read.xlsx(here("newbirds/Buteogallus_fragilis.xlsx"),sheet = "data"),
                 circus_hudsoni_tmt = read.xlsx(here("newbirds/Circus_hudsoni.xlsx"),sheet = "data"),
                 spizaetus_grinnelli_tmt = read.xlsx(here("newbirds/Spizaetus_grinnelli.xlsx"),sheet = "data")
                 )

for (i in 1:4) {
  dbWriteTable(con,name = names(newbirds)[i],value = newbirds[[i]],row.names=F,overwrite=T)
}
dbSendQuery(con,paste0("insert into tablenames (tablenames) values ('",paste0(names(newbirds)),"');"))
...
dev.print(pdf,"Figures and Tables/Final submission/Figure2_2col.pdf",width=15/cm(1),height=3/cm(1))
```
...
for (m in M:length(TSresults5)){
  for (n in N:length(TSresults5[[m]])){
    for (p in P:length(TSresults5[[m]][[n]])){
      ...
      mods<-compareModels(
        opt.joint.GRW(ts,pool=F),
        opt.joint.URW(ts,cl=list(fnscale=-1.00000001)),
...
```{r}
model.results5<-model.results10<-data.frame(species=NA,element=NA,measurement=NA,GRW=NA,URW=NA,Stasis=NA,Punc=NA,bestModel=NA)
M<-1;N<-1;P<-1;i<-1
for (m in M:length(TSmodels5)){
  ...
  for (n in N:length(TSmodels5[[m]])){
    ...
    for (p in P:length(TSmodels5[[m]][[n]])){
      ...
      wts5 <- TSmodels5[[m]][[n]][[p]]$Akaike.wt
      bestModel5<-rownames(TSmodels5[[m]][[n]][[p]])[which.max(TSmodels5[[m]][[n]][[p]]$Akaike.wt)]
      model.results5[i,]<-c(species,element,measurement,wts5,bestModel5)
      wts10 <- TSmodels10[[m]][[n]][[p]]$Akaike.wt
      bestModel10<-rownames(TSmodels5[[m]][[n]][[p]])[which.max(TSmodels10[[m]][[n]][[p]]$Akaike.wt)]
      model.results10[i,]<-c(species,element,measurement,wts10,bestModel10)
      i<-i+1
    }
  }
}
...
```{r}
param.results <- data.frame(array(dim=c(228,14)))
data <- TSresults5.pca
M<-1;N<-1;P<-1;i<-1
for (m in M:length(data)){
  species<-names(data[m])
  for (n in N:length(data[[m]])){
    element<-names(data[[m]][n])
    for (p in P:length(data[[m]][[n]])){
      measurement=names(data[[m]][[n]][p])
      temp <- modelRepTS(data[[m]][[n]][[p]]$paleoTS)
      param.results[i,] <- c(species,element,measurement,unlist(temp$params))
      i <- i + 1
    }
  }
}
colnames(param.results) <- c("species","element","measurement",names(unlist(temp$params)))
```
##make allBestFits and bestfits
```{r}
model.results<-data.frame(rbind(data.frame(model.results5,L=5),data.frame(model.results10,L=9)))
save(model.results,TSresults5,TSresults10,TSresults5.pca,TSresults10.pca,TSmodels5,TSmodels10,file="originalResults.RData")
...
```{r}
meas10<-unique(subset(allBestFits,!grepl("^PC",allBestFits$measurement)&allBestFits$L==9))
meas5<-unique(subset(allBestFits,!grepl("PC",allBestFits$measurement)&allBestFits$L==5))
pc10<-unique(subset(allBestFits,grepl("PC",allBestFits$measurement)&allBestFits$L==9))
pc5<-unique(subset(allBestFits,grepl("PC",allBestFits$measurement)&allBestFits$L==5))
```

## normalize Stasis.omega to variance
```{r}
#all5.joined and all10.joined are joins of the appropriate param.results with the appropriate subset of allBestFits
#they contain the model weights and parameters

data <- all5.joined
data$Stasis.omega.normalized <- NA
data$timestep <- NA
timeseries.set <- TSresults5.pca
for (i in 1:nrow(data)){
  m <- which(names(timeseries.set) == data[i,"species"])
  n <- which(names(timeseries.set[[m]]) == data[i,"element"])
  p <- which(names(timeseries.set[[m]][[n]]) == data[i,"measurement"])
  if (length(m) > 0 & length(n) > 0 & length(p) >0){
    data$timestep[i] <- mean(diff(timeseries.set[[m]][[n]][[p]]$paleoTS$tt),na.rm=T)
    #if (data[i,"L"] == 5) b <- 4:5 else b <- 7:9
    #var <- mean(timeseries.set[[m]][[n]][[p]]$paleoTS$vv[b],na.rm = T)
    #data[i,"Stasis.omega.normalized"] <- sqrt(abs(data[i,"Stasis.omega"]/var))
  }
}
data -> all5.joined

save(all5.joined,all10.joined,file="parameter_datasets.R")
load(file="parameter_datasets.R")

write.csv(data.frame(rbind(all5.joined,all10.joined)),row.names = F, file = "Figures and Tables/Second submission/Supplementary Table E - Model Parameters.csv")
```

## examine normalized stasis.omega parameter
```{r}
p <- ggplot(rbind(all10.joined,all5.joined),aes(x=bestModel,y=Stasis.omega.normalized)) + facet_grid(cols = vars(L)) + geom_boxplot()
...
```
###special plot function for Figure 4 and 6
```{r}
makeplot2 <- function(i,savePDF=F,target="Figures and Tables/Final submission"){
  #for figures 4 and 6
  all.series<-dplyr::distinct(bestfits[,1:2])
  filename<-paste0(target,"/",
                   as.character(all.series$species[i]),"_",
                   as.character(all.series$element[i]),".pdf")
  nspec<-length(which(meas[[as.character(all.series$species[i])]][[as.character(all.series$element[i])]]$Pit%in%goodPits))
  titletext<-paste0("''~italic('",
  ...
  #plot first Z dimensions
  Z <- 4
  mat <- t(matrix(1:Z,ncol=2))
  
  layout(mat)
  #plot named dimensions individually
  for (z in 1:(Z - 1)) {
    bgcol <- colorpairs[which(colorpairs$model == hlMods[z]),"Meas"]
    par(mar = c(2.1,2.1,3,1))
    plot(0,0,type = "n",ann = F,axes = F)
  ...
  #plot all PCs together
  z <- Z
  bgcol <- "white"
  percvartext <- sapply(percvar, function(x) paste0(round(x,2) * 100, "%", collapse = ""))
  linecols <- colorpairs[match(hlMods[z:length(tsdata)],colorpairs$model),"Line"]
  linewidth <- c(1.5,1,0.5)
  ...
  rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],
       col = bgcol)
  par(mar = c(2.1,2.1,3,1),new = TRUE)
  tsdata[[length(tsdata)]]$paleoTS$label <- 
    paste0("PCs 1-", length(percvar))
  plot(tsdata[[length(tsdata)]]$paleoTS,xaxt = "n",col = linecols[length(tsdata)-3], lwd = linewidth[length(tsdata)-3],
       ylim = paleoTS.plot.ylims(tsdata[[z]]$paleoTS))
  for (j in (length(tsdata) - 1):z) {
    tsdata[[j]]$paleoTS$label <- ""
    plot(tsdata[[j]]$paleoTS,add = T,col = linecols[j - z + 1], lwd = linewidth[j - z + 1])
  }
  abline(v = unique(unlist(climateBins)),lty=2,lwd=0.5)
  axis(1,at=unique(unlist(climateBins)),
       labels=unique(unlist(climateBins))/(-1000),cex=0.9)
  legendtext <- apply(rbind(names(percvartext),percvartext),2,function(x) paste0(x[1]," (",x[2],")"))
  legend(x = "topleft",legend = legendtext,col = linecols,lwd = linewidth,bty = "n")

  mtext(parse(text=titletext),side=3,line=-2,outer=T)
  if (savePDF==T) {dev.print(pdf,filename,width=ncol(mat)*3,
          height=nrow(mat)*3)}
}

```
#### print figure 4 and 6
```{r}
all.series<-dplyr::distinct(bestfits[,1:2])
figlines <- which(all.series$species%in% c("Asio_otus","Tyto_alba","Athene_cunicularia","Haliaeetus_leucocephalus"))
for (i in 1:4) makeplot2(figlines[i],savePDF = T)
```
...
colorpairs <- data.frame(model=c("GRW","Punc","Stasis","URW"),
                  ...
                  Line = c("mediumorchid","salmon3","turquoise4","goldenrod4"))
```
###print all
```{r}
...
dev.print(pdf, "Figures and Tables/Fig3_2col.pdf", width=15/cm(1), height=9.5/cm(1))
```
##new pit age ranges
```{r}
source("setup.R")
...
dev.print(pdf, "Figures and Tables/Final submission/Figure1_2col.pdf", width=15/cm(1), height=9.4/cm(1))
```
...
##GRW results (Supp. Table D)
```{r}
Table4<-data.frame(apply(subset(allBestFits,allBestFits$bestModel=="GRW")[,1:3],1:2,as.character))
...
longTable4$mstep <- NA
for (i in 1:nrow(longTable4)) {
  j <- which(all10.joined$species==longTable4$species[i] &
          all10.joined$element==longTable4$element[i] &
          all10.joined$measurement==longTable4$measurement[i])
  longTable4$mstep[i] <- all10.joined$GRW.mstep[j] * 10 / all10.joined$timestep[j] * 1000000
}
longTable4 -> Table4
...
Table4<-dplyr::distinct(Table4[,c("species","element","measurement","direction","mstep")])
...
write.csv(Table4,file=here("Figures and Tables/Second submission/Supplementary Table D - Trends.csv"),row.names = F)
```
...
#dump data files for Dryad
```{r}
tablenames <- unlist(dbGetQuery(con,"select * from tablenames;"))
for (i in 1:length(tablenames)) {
  filename <- paste0(here("Figures\ and\ Tables/Final\ submission/datafiles/"),tablenames[i],".csv")
  table <- dbGetQuery(con, paste0("select * from ",tablenames[i],";"))
  write.csv(table, file=filename, row.names = F)
}
for (i in 1:4) {
    filename <- paste0(here("Figures\ and\ Tables/Final\ submission/datafiles/"),names(newbirds)[i],".csv")
  write.csv(newbirds[[i]],file=filename,row.names=F)
}
```
# extract chronologies for Neotoma
```{r}
load("oxcalResults/oxcalResults.Rdata")
neotomaids <- read.csv("neotoma-ids.csv")
result <- vector(mode = "list",length = length(oxcalResults))
names(result) <- names(oxcalResults)
I <- 1
for (i in I:length(oxcalResults)){
  full <- oxcalResults[[i]]
  if (names(pit[i]) %in% neotomaids$pitnumber) {
      AUages <- data.frame(analysisunitid = rep(subset(neotomaids$analysisunitid, neotomaids$pitnumber == names(pit[i])),2))
  } else  {
    AUages <- data.frame(analysisunitid = rep(names(pit[i]),2))
  }
  AUages$chronology.agemodel <- c(
    rep("event",length(AUages$analysisunitid)/2),
    rep("bounds",length(AUages$analysisunitid)/2))
  AUages$sampleages.age <- c(medianProbDist(full$Event$posterior_probabilities,
                                            quantile = 0.5),"NA")
  AUages$sampleages.ageolder <- c(
    medianProbDist(full$Event$posterior_probabilities,quantile = 0.3173),
    medianProbDist(full$begin$posterior_probabilities,quantile = 0.5))
  AUages$sampleages.ageyounger <- c(
    medianProbDist(full$Event$posterior_probabilities,quantile = 0.6827),
    medianProbDist(full$end$posterior_probabilities,quantile = 0.5))
  controls <- data.frame(geochronid = rep(NA,length(grep("\\[",names(full),value = T))))
  controls$geochronology.labnumber <- sapply(grep("\\[",names(full),value = T),
       function(x) strsplit(strsplit(x,split = "\\[")[[1]],split = "\\]")[[2]])
  controls$chroncontrols.age <- sapply(full[grepl("\\[",names(full))],
                       function(x) medianProbDist(x$posterior_probabilities,quantile = 0.5))
  controls$chroncontrols.agelimitolder <- sapply(full[grepl("\\[",names(full))],
            function(x) medianProbDist(x$posterior_probabilities,quantile = 0.0455))
  controls$chroncontrols.agelimityounger <- sapply(full[grepl("\\[",names(full))],
            function(x) medianProbDist(x$posterior_probabilities,quantile = 0.9545))
  result[[i]] <- list(sampleages = AUages,chroncontrols = controls)
}
```
